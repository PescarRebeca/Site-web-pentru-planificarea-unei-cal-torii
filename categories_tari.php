...
if (isset($_GET['type']) && $_GET['type'] != '') {
    ...
    if ($type == 'status') {
        ...
        $update_status_sql = "UPDATE tari SET status='$status' WHERE id='$id'";
    ...
    if ($type == 'delete') {
        $id = get_safe_value($con, $_GET['id']);
        $delete_sql = "DELETE FROM tari WHERE id='$id'";
          ...
          <div class="col-xl-12">
             <div class="card">
                <div class="card-body" style="padding-left:80px;">
                   <h4 class="box-title">Țări</h4>
                   <h4 class="box-link"><a href="manage_tari.php">Adaugă Țară</a></h4>
                </div>
                <div class="card-body--">
    ...
    <tbody>
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            <tr>
                ...
                <td><img src="<?php echo $row['imagine'] ?>" alt="Imagine țară" style="width: 100px;"></td>
                <td><?php echo truncateText($row['relief'], 50) ?></td>
                <td><?php echo truncateText($row['clima'], 50) ?></td>
                <td><?php echo truncateText($row['istoric'], 50) ?></td>
                <td data-description-id="<?php echo $row['id'] ?>">
                    <div class="truncate-text"><?php echo truncateText($row['descriere'], 50) ?></div>
                    <?php if (strlen($row['descriere']) > 50): ?>
                        <button class="show-more" data-id="<?php echo $row['id'] ?>">Afișează mai mult</button>
                        <div class="full-description" style="display: none;"><?php echo $row['descriere'] ?></div>
                    <?php endif; ?>
                </td>
                ...
                <td>
                    <?php
                    echo "<span class='badge badge-edit'><a href='manage_tari.php?id=" . $row['id'] . "'>Editare</a></span>&nbsp;";
