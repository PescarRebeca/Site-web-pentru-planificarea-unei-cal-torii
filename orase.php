 ...
 <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyD_VBbuciYSpXgHniGJSAbibwC66QnGWjg&callback=initMap" async defer></script>
    <title>
        ...
        if(isset($_GET['city'])) {
            ...
            echo $appName . ' - ' . "Informații despre orașe";
        }
        ?>
...
.right-column .button {
    background-color:#ffc107; /* Culoare de fundal a butoanelor */
...
/* Stiluri existente pentru ecrane mari rămân neschimbate */

/* Adăugăm stiluri pentru ecrane mici și medii */
@media screen and (max-width: 970px) {
    .wrapper .btn {
        display: block;
    }
...
/* Ajustări pentru flexbox în secțiunea .container2 */
@media screen and (max-width: 970px) {
    .container2 {
        flex-direction: column;
    }
...
<div class="container2">
 ...
 <form method="post" action="adauga_in_favorite.php" class="favorite-form" id="favoriteForm">
    ...
    <button type="submit" name="add_to_favorites">Adaugă <?php echo htmlspecialchars($city); ?> în Favorite</button>
</form>
<div id="message"></div>
            <div class="submenu">
    <a href="#" class="submenu-item" data-content="aboutContent">Despre</a>
       <a href="#" class="submenu-item" data-content="DestinationsContent">Destinații</a>
 ...
 <?php
   // Include the database connection file
    ...
    if(isset($_GET['city'])) {
        ...
        // Query to get city details from the database
        $sql = "SELECT * FROM destinatii WHERE oras = '$city'";
        $result = $conn->query($sql);
        ...
        if ($result->num_rows > 0) {
            // Display city details
            while($row = $result->fetch_assoc()) {
                echo "<h1>" . $row["nume_destinatie"] . "</h1>";
                
                echo "<p>" . $row["descriere"] . "</p>";
                 echo "<img src='" . $row["imagine"] . "'  class='city-image'>";
...
<div class="city-break-container">
    <img src="2.jpg" alt="City Break" class="city-break-image">
    <div class="city-break-info">
        <h3>City Break <?php echo $city; ?></h3>
        <button onclick="redirectToCityBreakPage()">Mai multe detalii</button>
    </div>
</div>
...
<div class="city-break-container">
    <img src="2.jpg" alt="City Break" class="city-break-image">
    <div class="city-break-info">
        <h3>Tur 1 zi <?php echo $city; ?></h3>
        <button onclick="redirectToTourPage()">Mai multe detalii</button>
    </div>
</div>
</div>
...
<script>
    ...
    var url = 'citybreak.php?city=<?php echo urlencode($city); ?>';
    window.location.href = url;
}

     function redirectToTourPage() {
    var url = 'citytour.php?city=<?php echo urlencode($city); ?>';
    window.location.href = url;
}
